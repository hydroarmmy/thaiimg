...
document.querySelectorAll('.format-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    convertBtn.firstChild.nodeValue = `แปลง HEIC → ${outputFormat.toUpperCase()} `;
  });
});
...
function addFiles(files) {
  const valid = files.filter(isHeic);
...
convertBtn.addEventListener('click', async () => {
  if (convertBtn.classList.contains('done')) {
    ...
    convertBtn.innerHTML = `แปลง HEIC → ${outputFormat.toUpperCase()} <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><circle cx="9" cy="9" r="7.5" stroke="white" stroke-width="1.5"/><path d="M6 9 L12 9 M9.5 6.5 L12 9 L9.5 11.5" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
    resetPage();
    return;
  }
  ...
  for (let i = 0; i < selectedFiles.length; i++) {
    ...
    try {
      // heic2any handles jpeg & png natively; for webp we go HEIC→PNG→WEBP via canvas
      let blob;
      if (info.mime === 'image/webp') {
        const png = await heic2any({ blob: file, toType: 'image/png' });
        blob = await canvasReencode(Array.isArray(png) ? png[0] : png, 'image/webp', info.quality);
      } else {
        const r = await heic2any({ blob: file, toType: info.mime, quality: info.quality ?? 0.92 });
        blob = Array.isArray(r) ? r[0] : r;
      }
      const outName = file.name.replace(/\.[^/.]+$/, '') + '.' + info.ext;
...
function canvasReencode(blob, mime, quality) {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      c.toBlob(b => b ? resolve(b) : reject(new Error('Canvas failed')), mime, quality);
    };
